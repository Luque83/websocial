...
/**
 * Obtiene el catálogo de trabajadores de la plantilla de la entidad
 */
export async function getOrgStaffCatalogAction(): Promise<Worker[]> {
  try {
    ...
    // Buscar si hay una matriz guardada en project_tools con slug 'gestion-personal'
    ...
    if (
      toolRecord?.data &&
      Array.isArray((toolRecord.data as PersonalMatrixData).workers) &&
      (toolRecord.data as PersonalMatrixData).workers!.length > 0
    ) {
      return (toolRecord.data as PersonalMatrixData).workers!;
    }
